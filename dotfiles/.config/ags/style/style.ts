...
const { theme, effects, layout, font } = options.look;

const { scheme, dark, light } = theme;

const { shadows, blur, widget } = effects;

const { padding, spacing, radius, border } = layout;

const popoverPaddingMultiplier = 1.6;

// eslint-disable-next-line @typescript-eslint/no-explicit-any
const t = (dark: Option<any> | string, light: Option<any> | string) =>
    scheme.value === "dark" ? `${dark}` : `${light}`;

// eslint-disable-next-line @typescript-eslint/no-explicit-any
const $ = (name: string, value: string | Option<any>) => `$${name}: ${value};`;
